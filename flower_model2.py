...
# 로더에 대한 몇가지 매개변수를 정의
batch_size = 120  # 한번에 학습할 갯수(기본값 : 32 / 데이터크기가 크면 학습속도은 높아지나, 메모리 사용량이 급격히 증가함)
...
# image_batch : (128,180, 180, 3) 모양의 텐서 / 180x180x3(3 : 컬러이미지를 의미함) 모양의 랜서를 한번에 128개씩 배치
# labels_batch (128,) 모양의 텐서이며 128개의 이미지에 해당하는 레이블(즉, 정답)
for image_batch, labels_batch in train_ds :
    print("image_batch", image_batch.shape)
    print("labels_batch", labels_batch.shape)
    break

# 예상값을 8부터 255까지 저장하기 위해 이미지 값 전처리
...
# ######################################################
# # 3. 학습 모델 컴파일
# # 일반적으로 학습 모델을 만들때 학습데이터의 80%는 학습용, 20%는 검증용으로 사용
# ######################################################
#
...
# ######################################################
# # 6. 학습 모델 파일 생성
# # 파일확장자 없는 파일, SaveedModel 포맷 - TensorFlow 2.x 기본 학습모델
# # h5 확장자 파일 : HDFS 포맷을 의미 - Keras 기본 학습 모델
# ######################################################
#
...
######################################################
# 8. 데이터 증가
# 학습모델의 정확도 성능 부족으로 강화를 위해 데이터 증가
######################################################
