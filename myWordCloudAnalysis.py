...
# 설치한 matplotlib 외부라이브러리의 pyplot 기능을 사용하며, RXPlot 기능을 약어로 정의
...
text = "연합뉴스 한국폴리텍대학 서울강서캠퍼스(학장 노정진)는 데이터분석과 2학년 김한결, 황윤영, 홍두표 학생과 1학년 김도우 학생이" \
        "한이음 공모전에서 장려상을 받아 세종대학교에서 개최된 '소프트웨어(SW) 인재페스티벌'에 초청받았다고 18일 밝혔다. " \
...
# join 함수를 사용하며, analysis_text 변수에 문자열로 변환된 결고를 저장함
analysis_text = (" ".join(myHannanum.nouns(replace_text)))

# stopwords변수에 원하지 않는 단어들 추가
