...
# K-NN(k-Nearset Neighbour) 알고리즘 객체 생성
knn = cv2.ml.KNearest_create()

# 힌번에 학습시킬 데이터단위(한번당 1000건씩 학습량 증가)
train_idx = 1000;
...
for i in range(1, for_cnt):
    ...
    ret, result, neighbours, dist = knn.findNearest(test_data[:sample_cnt], k=5) # K-NN 분류 수행
    accur = sum(test_label[:sample_cnt] == result.flatten()) / len(result) # 성능 측정
